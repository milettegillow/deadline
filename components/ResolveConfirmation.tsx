...
import type { ResolveResult } from "@/lib/complete";

/** Friendly confirmation page shared by the public /done and /skip routes. */
export default function ResolveConfirmation({
  result,
}: {
...
function present(result: ResolveResult): {
  emoji: string;
  heading: string;
  body: string;
} {
  switch (result.status) {
    ...
    case "skipped":
      return {
        emoji: "⏭️",
        heading: "No worries — skipped",
        body: `“${result.title}” skipped this time. It'll come back round next cycle if it recurs.`,
      };
    case "already_done":
      return {
        emoji: "👍",
    ...
    case "already_skipped":
      return {
        ...
        body: `“${result.title}” was already skipped for this cycle.`,
      };
    case "not_found":
      return {
        emoji: "🤔",
    ...
    case "error":
      return {
        ...
        body: "We couldn't update this just now. Please try again shortly.",
      };
  }
}
